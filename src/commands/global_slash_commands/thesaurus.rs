...
async fn handler (command_interaction: &ApplicationCommandInteraction, _context: &Context, _interaction: &Interaction) -> Result<CommandSuccess, CommandError>{
    ...
    let json = bot::HttpClient::https_get_json_with_headers(thesaurus_url, vec![("X-Api-Key",ninja_facts_key.as_str())]).await.to_command_result()?;
    ...
    let response = match response {
        Err(e) => {
            ...
            if let Ok(v) = serde_json::from_str::<ThesaurusErrorResponse>(json.as_str()) {
                return Err(CommandError::InvalidUserInputError(format!("Thesaurus api error: {}", v.error)));
            }

            return Err(CommandError::InternalError(format!("Could not understand api response. Details: '{:?}'\nOffending json: '{}'", e, json)));
        },
        Ok(v)=> v
    };
    ...
    let reply = {
        ...
        reply.write("***Entry for***: ").writeln(response.word.as_str())
            .write("***Synonyms***: ");
