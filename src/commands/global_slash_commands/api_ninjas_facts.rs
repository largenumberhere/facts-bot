...
async fn handler(
    ...
    let uri: reqwest::Url = "https://api.api-ninjas.com/v1/facts?limit=1"
        .parse()
        .to_command_result_with("url failed to convert")?;
    let ninja_facts_key = bot::get_token_from("api-ninjas-com-key.file".to_string())
        .await
        .to_command_result_with("Failed to get ninja facts key")?;
    ...
    .await
    .to_command_result_with("Http request with headers failed")?;

    let facts: Vec<FactReply> = serde_json::from_str(json.as_str()).to_command_result()?;
    let fact_reply = facts
        .get(0)
        .ok_or_else(|| "Failure to get fact 0".to_string())
        .to_command_result()?;

    Ok(CommandSuccess::SuccessWithReply(
        fact_reply.fact.to_string(),
    ))
}
