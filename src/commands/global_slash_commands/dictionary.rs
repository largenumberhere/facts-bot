...
async fn handle(
    ...
    let url: Url = format!("{}{}", address, &string)
        .parse()
        .to_command_result()?;

    let token = bot::get_token_from("api-ninjas-com-key.file".to_string())
        .await
        .to_command_result()?;
    let json =
        bot::HttpClient::https_get_json_with_headers(url, vec![("X-Api-Key", token.as_str())])
            .await
            .to_command_result()?;
