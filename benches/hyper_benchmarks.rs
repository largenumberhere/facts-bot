...
pub fn criterion_benchmark(c: &mut Criterion){
    ...
    assert!(executor.block_on(hyper_client_create_and_send_once()) .is_ok());
    assert!(executor.block_on(hyper_client_create_and_send_twice()).is_ok());
    ...
    group.bench_function("hyper-http-0",|b|  b.to_async(&executor)
        .iter(|| hyper_client_create()));

    group.bench_function("hyper-http-1",|b|  b.to_async(&executor)
        .iter(|| hyper_client_create_and_send_once()));

    group.bench_function("hyper-http-2", |b| b.to_async(&executor)
