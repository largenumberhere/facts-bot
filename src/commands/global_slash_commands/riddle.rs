...
async fn handler(
    ...
    let ninja_facts_key = bot::get_token_from("api-ninjas-com-key.file".to_string())
        .await
        .to_command_result()?;
    let url: Url = "https://api.api-ninjas.com/v1/riddles"
        .parse()
        .to_command_result()?;
    ...
    .await
    .to_command_result()?;
    let response: TriviaResults = serde_json::from_str(json.as_str())
        .to_command_result_with("failed to serialize response")?;
    ...
    let reply = {
        ...
        reply
            .write("***Title:***")
            .writeln(response0.title.as_str())
            .write("***Q:*** ")
            .writeln(response0.question.as_str())
            .write("***A:*** ||")
            .writeln(response0.answer.as_str())
            .write("||");
        reply
    };
