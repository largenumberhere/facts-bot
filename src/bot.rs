...
impl EventHandler for CommandsDetails {
    async fn ready(&self, context: Context, bot_data: Ready) {
        ...
        let current_commands = Command::get_global_application_commands(&context.http)
            .await
            .unwrap();
        ...
        for new_command in self.context_menu_commands.iter() {
            if current_commands
                .clone()
                .find(|c| c.name == new_command.name)
                .is_some()
            {
                match new_command.force_command_update {
                    Some(_) => {
                        println!(
                            "Warning: command with name '{}' is being forced to re-upload",
                            new_command.name
                        )
                    }
                    None => {
            ...
            let result =
                Command::create_global_application_command(&context.http, |command_builder| {
                    command_builder
            ...
            match result {
                ...
                Err(ref e) => {
                    failed_commands += 1;
                    eprintln!(
                        "failed to create context menu command!\ncommand_result:{:#?}\nerror:{}",
                        result, e
                    );
                }
            }
        ...
        let old_slash_commands = {
            ...
            for command in current_commands.clone() {
                match current_commands.clone().find(|ac| ac.id == command.id) {
                    Some(_v) => {}
                    None => old_commands.push(command),
                }
            }
            //}
        ...
        for unused_slash_command in old_slash_commands {
            match Command::delete_global_application_command(&context.http, unused_slash_command.id)
                .await
            {
                Err(e) => eprintln!(
                    "Failed to delete command with id: '{}', name: '{}'\n error:{:#?}",
                    unused_slash_command.id, unused_slash_command.name, e
                ),
                Ok(_) => println!(
                    "Deleted command with id: '{}', name: '{}',",
                    unused_slash_command.id, unused_slash_command.name
                ),
            }
        }
    ...
    async fn interaction_create(&self, context: Context, interaction: Interaction) {
        match interaction {
            Interaction::ApplicationCommand(ref c) => {
                println!(
                    "command of type {:#?}, received from {}:{}",
                    c.data.kind, &c.user.name, c.user.discriminator
                );
                ...
                match c.data.kind.borrow() {
                    ...
                    CommandType::User => {
                        //Context menu command, found when clicking on user and Apps
                        handle_context_menu_command_interaction(self, &context, &interaction, &c)
                            .await;
                    }
                    CommandType::Message => {} //
                    CommandType::Unknown => {}
...
async fn handle_context_menu_command_interaction(
    command_details: &CommandsDetails,
    context: &Context,
    interaction: &Interaction,
    command: &ApplicationCommandInteraction,
) {
    let command_name_requested = command.data.name.as_str();
    let command_found = command_details
        .context_menu_commands
        .iter()
        .find(|c| c.name.as_str() == command_name_requested);
    let command_found = match command_found {
        None => {
            command
                .quick_reply("not a valid command!".to_string(), &context.http())
                .await;
            return;
        }
        Some(v) => v,
    ...
    match command_processing_result {
        ...
        Err(e) => match e {
            CommandError::InvalidUserInputError(e) => {
                command
                    .quick_reply(
                        format!(":( Sorry we couldn't parse your data because: {}", e),
                        &context.http,
                    )
                    .await;
            }
            CommandError::InternalError(e) => {
                ...
                command
                    .quick_reply(
                        format!(":( sorry, your request failed because: {}", e),
                        &context.http,
                    )
                    .await;
            }
...
async fn handle_slash_command_interaction(
    command_details: &CommandsDetails,
    context: &Context,
    interaction: &Interaction,
    command: &ApplicationCommandInteraction,
    ...
    let command_name_requested = command.data.name.as_str();
    let bot_command = command_details
        .slash_commands
        .iter()
        .find(|c| c.name.as_str() == command_name_requested);
...
static TOKENS_CACHE: once_cell::sync::Lazy<Mutex<HashMap<String, String>>> =
...
pub async fn get_token_from(file_name: String) -> Result<String, std::io::Error> {
    ...
    let file_contents = match std::fs::read_to_string(&file_name) {
        ...
        Err(e) => {
            eprintln!(
                "Required key file failed to open! file: '{}' error: {:#?}",
                file_name, e
            );
            return Err(e);
        }
    ...
    if file_contents.chars().any(|c| c.is_whitespace()) {
        return Err(std::io::Error::new(
            ...
            format!("Illegal whitespace found in key!"),
        ));
    }

    {
        ...
        if let Some(v) = result {
            panic!(
                "key '{}' had value '{}' and was unintentionally overwritten with '{}'",
                &file_name, v, &file_contents
            );
        }
    }
...
impl QuickReplyEphemeral for &ApplicationCommandInteraction {
    async fn quick_reply_ephemeral(&self, text: String, http: &Http) {
        ...
        match result {
            ...
            Err(e) => {
                eprintln!(
                    "Failed to respond to {}:{}, because {:#?}",
                    self.user.name, self.user.discriminator, e
                );
            }
        }
    }
...
impl HttpClient {
    ...
    pub async fn https_get_json_with_headers(
        uri: reqwest::Url,
        ...
        for header in headers {
            let header_value = header
                .1
                .parse()
                .into_bot_error("failed to parse https header")?; //header.1.parse().to_bot_error("failed to insert headers")?;//.to_bot_error(format!("failed to insert ({}, {}) into headers", header.0, header.1).as_str())
        ...
        let response = client
            ...
            .send()
            .await;
        let response = response.into_bot_error(
            format!(
                "failed to post with headers '{:#?}' to {:?}",
                header_map, uri
            )
            .as_str(),
        )?;
        let json = response
            .text()
            .await
            .into_bot_error("failed to get text from response")?;
        Ok(json)
    }
}
    ...
    E: std::error::Error,
{
    fn into_bot_error(self: Result<T, E>, message: &str) -> Result<T, BotError> {
        match self {
            ...
            Err(e) => Err(BotError::new(message.to_string(), &e)),
        }
    }
}
...
impl Display for BotError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Error :'{}'. Details: {:#?}",
            self.error_details, self.base_error
        )
    }
}
