...
async fn handler(
    ...
    if option0.len() > 1000 {
        return Err(CommandError::InvalidUserInputError(
            "Sampple is too long. Max 2000 characters allowed".to_string(),
        ));
    }
    ...
    if words < 10 {
        return Err(CommandError::InvalidUserInputError(
            "Sample is too short. Min 10 characters.".to_string(),
        ));
    }
    ...
    let reply = {
        ...
        reply
            .write("***Language name:*** ")
            .writeln(valid_response.language.as_str())
            .write("***Language Id (ISO):*** ")
            .writeln(valid_response.iso.as_str());
