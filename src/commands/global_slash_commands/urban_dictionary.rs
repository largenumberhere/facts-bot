...
async fn handler(
    ...
    let url: Url = format!("https://api.urbandictionary.com/v0/define?term={}", option)
        .parse()
        .to_command_result()?;
    let json = bot::HttpClient::https_get_json(url)
        .await
        .to_command_result()?;

    let response: UrbanDictionaryResponse =
        serde_json::from_str(json.as_str()).to_command_result()?;
    //println!("{:#?}", response);

    let mut reply = String::new();
    reply
        .write("***Top definition for ")
        .write(option.as_str())
        .writeln("***");

    let definition = match response.list.first() {
        None => {
            return Ok(CommandSuccess::SuccessWithReply(format!(
                "No entry found for {}",
                option
            )));
        }
    ...
    };
    {
        ...
        let time = match time {
            ...
            Err(e) => {
                return Err(CommandError::InternalError(format!(
                    "failed to convert time '{}' becuase of parsing error '{:#?}'",
                    definition.written_on, e
                )));
            }
        };
        ...
        let author = {
            if definition.author.len() == 0 {
                ...
                //Bug: it refuses to compile if this if statement is not used
        ...
        reply
            ...
            .write(" ")
            .write("<t:")
            .write(time.timestamp().to_string().as_str())
            .writeln(">")
            .write("Votes ")
            .write(definition.thumbs_up.to_string().as_str())
            .write(" | ")
            .writeln(definition.thumbs_down.to_string().as_str())
...
struct UrbanDictionaryDefinition {
    ...
    ///what the hell is this for? Why is it empty???
