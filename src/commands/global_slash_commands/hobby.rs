...
impl GetSlashCommandDetails for HobbySlashCommand {
    fn get_slash_command_details() -> GlobalSlashCommandDetails {
        GlobalSlashCommandDetails{
            ...
            description: "Get a random hobby suggestion from https://api-ninjas.com/api/hobbies".to_string(),
            options: vec![],
...
async fn handler (_command_interaction: &ApplicationCommandInteraction, _context: &Context, _interaction: &Interaction) -> Result<CommandSuccess, CommandError>{
    ...
    let url:Url = "https://api.api-ninjas.com/v1/hobbies?category=general".parse().to_command_result()?;
    ...
    let reply =  {
        ...
        reply.writeln("***Hobby suggestion***")
            .write("***Name:*** ").writeln(response.hobby.as_str())
            .write("***Link*** ").writeln(response.link.as_str());
