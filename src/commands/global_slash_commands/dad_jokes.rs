...
async fn handler(
    ...
    let ninja_facts_key = bot::get_token_from("api-ninjas-com-key.file".to_string())
        .await
        .to_command_result_with("failed to read api key from file on server")?;
    let url = "https://api.api-ninjas.com/v1/dadjokes?limit=1"
        .parse()
        .to_command_result_with("failed to parse dadjokes url")?;
    let reply = bot::HttpClient::https_get_json_with_headers(
        url,
        vec![("X-Api-Key", ninja_facts_key.as_str())],
    )
    .await
    .to_command_result_with("failed to read http reply json")?;
    let jokes: Jokes = serde_json::from_str(reply.as_str())
        .to_command_result_with("response from server was unclear. The JSON was a mess")?;
    let joke = jokes
        .get(0)
        .ok_or(())
        .to_command_result_with("there was no joke );")?;
    let joke = &joke.joke;
