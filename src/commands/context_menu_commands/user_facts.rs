...
use serenity::model::application::interaction::application_command::{
    ApplicationCommandInteraction, ResolvedTarget,
};
...
async fn handler(
    ...
    let target_info = command_interaction
        .data
        .target()
        .ok_or("Failed to fetch user info for the specified target!".to_string())
        .to_command_result()?;

    let (target_user, _partial_user_info) = match target_info {
        ResolvedTarget::User(user, b) => (user, b),
    ...
    match user.public_flags {
        ...
        Some(flags) => {
            reply.push_str(
                format!("These flags are on the user's account: {:#?}\n", flags).as_str(),
            );
        }
    }
    ...
    reply.push_str(
        format!(
            "The user was created at: <t:{}> (In your timezone), or {} unix time \n",
            timestamp, timestamp
        )
        .as_str(),
    );
    ...
    match user.banner_url() {
        ...
        None => match user.accent_colour {
            Some(colour) => reply.push_str(
                format!("The user has the banner colour hex #{}\n", colour.hex()).as_str(),
            ),
            None => {
    ...
    command_interaction
        .quick_reply_ephemeral(reply, &context.http)
        .await;
