...
async fn handler(_command_interaction: &ApplicationCommandInteraction, _context: &Context, _interaction: &Interaction) -> Result<CommandSuccess, CommandError> {
    ...
    let trivia_responses: TriviaResponses = serde_json::from_str(json.as_str()).to_command_result()?;
    let trivia_response: &TriviaResponse  = trivia_responses.get(0).ok_or("failed to get value 0 from trivia response json").to_command_result()?;

    let mut response = String::new();
    response.push_str(format!("Q: {}\n", trivia_response.question).as_str());
    response.push_str(format!("A: ||{}|| (click on black box to show)", trivia_response.answer).as_str());
