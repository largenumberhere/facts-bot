...
pub fn get_string_option(
    ...
    if options.len() != 1 {
        return Err(CommandError::InvalidUserInputError(format!(
            "Exactly 1 option expcted for command reponse. {} given",
            options.len()
        )));
    }
    ...
    let option0 = match options.first() {
        ...
        None => {
            return Err(CommandError::InternalError(format!(
                "First option did not exist but size was 1 waaaaa????! >~> options:'{:#?}'",
                options
            )))
        }
    };
    ...
    let option0_value = match &option0.value {
        ...
        None => {
            return Err(CommandError::InvalidUserInputError(format!(
                "First option did not have a type aaaaaaaaaaa. options:'{:#?}'\noption0:{:#?}",
                options, option0
            )));
        }
    };
    ...
    let option0_as_string = match option0_value {
        ...
        _ => {
            return Err(CommandError::InvalidUserInputError(format!(
                "First option must be of type String. Received: {}",
                option0_value
            )));
        }
    };
