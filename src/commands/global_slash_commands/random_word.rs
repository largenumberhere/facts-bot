...
async fn handler (command_interaction: &ApplicationCommandInteraction, context: &Context, interaction: &Interaction) -> Result<CommandSuccess, CommandError> {
    ...
    let word =  {
        let word_url: Url = "https://api.api-ninjas.com/v1/randomword".parse().to_command_result()?;

        let json = bot::HttpClient::https_get_json_with_headers(word_url, vec![("X-Api-Key", ninja_facts_key.as_str())]).await.to_command_result()?;
        let response: RandomWordResponse = serde_json::from_str(json.as_str()).to_command_result()?;
    ...
    let definiton = match dictionary_url {
        Some(url) => {
            let response =  bot::HttpClient::https_get_json_with_headers(url, vec![("X-Api-Key", ninja_facts_key.as_str())]).await.to_command_result()?;
            let response: DictionaryResponse = serde_json::from_str(response.as_str()).to_command_result()?;
